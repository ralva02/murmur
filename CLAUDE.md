...
A second, independent pipeline handles long-form **Recordings** (spec: `docs/superpowers/specs/2026-07-03-recordings-design.md`): `LongFormRecorder` (mic via AVAudioEngine + system audio via a CoreAudio process tap — the aggregate device MUST include a real output sub-device or the tap IOProc never fires) → `RecordingsStore` (one folder per recording under Application Support/Murmur/Recordings, same test-guard as AppStore) → `FileTranscriber` (SpeechAnalyzer file mode, full finalization) → `SummaryProvider` (Ollama default / Claude opt-in via `AnthropicClient`, key in Keychain, never settings.json). Summaries also emit a `TITLE:` line (applied unless the user renamed the recording) and list action items under an `## Action items` heading with `(@Owner)` markers; `TaskExtractor` reads that visible section (NOT a separate machine block — small local models put the format in the visible text instead) into `pendingTasks`, which a draft-then-confirm review sheet commits into `TasksStore` (a flat global Tasks section). Summary engines: Ollama · LM Studio (OpenAI-compatible, `OpenAICompatibleClient`) · Claude. `swift run Murmur --summarize "<transcript>"` exercises the summary→title→task chain against the live model. First use of the system tap and of the Downloads watcher each BLOCK on a TCC prompt — that is expected, not a hang. Sending sampling params (temperature) to Opus-class Claude models returns 400; `AnthropicClient` deliberately omits them.
